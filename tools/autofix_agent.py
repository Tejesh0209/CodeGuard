...
async def create_branch(repo_name: str, branch_name: str, from_branch: str = "main") -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        return f"ERROR: Could not create branch - {create_resp.status_code}"
...
Common fixes:
  ...
  N+1 Query:
    ...
    AFTER:  single JOIN query fetching everything at once

FileSystem MCP tools available:
  read_file(path)           -> read local files in the CodeGuard project
  write_file(path, content) -> write fixes to local files
  list_directory(path)      -> list files in a directory
Use FileSystem MCP to read local coding standards or style guides.
Prefer GitHub API tools for reading/writing actual repo files.

Once the Draft PR is created, report:
- Which issues were fixed
- The Draft PR URL
- Any issues that could not be safely auto-fixed and why
"""
...
1. Call get_file_contents("{repo_name}", "app.py", "{pr_branch}")
...
5. Call commit_fix with the fixed content and the SHA from step 1
    ...
    filesystem_mcp = MCPServerStdio(
        params=MCPServerStdioParams(
            ...
            args=[
                ...
                "/Users/tejeshboppana/Desktop/CodeGuard"
            ]
        ),
        name="filesystem-mcp"
    )
